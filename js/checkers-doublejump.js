		...
		for (var r=0; r<row; r++){
			...
			for (var c=0; c<column; c++){
				...
				if( (r<3&&c<3) || (r>8&&c<3) || (c>8&&r<3) || (c>8&&r>8) ) 	
				{
					...
					// This can be cleaned up I think using a toggleClass command
					// Basically what it does is decide, using a flag, whether 
					// a space is legal or illegal.  
					switch (flag)
					{ 	case true:
...
/***********************************************************************/
	...
	function takePiece (old, current_color, current_kind, move1, move2, current, takeFlag) {
		...
		// console.log('movieng to jumpPrompt');
		// jumpPrompt(old, newSquare); 

		// turn === 1 ? post=4 : post = turn-1;
		// player[post].score++;
		// addScorePrompt();
	...
	function checkDouble (current) {
		
		possibleMoves(current);
		removeOneClass(move1);
		removeOneClass(move2);

		if ((!(isOccupied(move1))) && (!(isOccupied(move2)))) 


		if ((isOccupied(move1) && !(isOccupied(move1_2))) || (isOccupied(move2) && !(isOccupied(move2_2)))) {nextTurn()}
		
		
		else if ((isOccupied(move1)) && !(isOccupied(move1_2))) { 
			//jumpable(move1,move1_2);
			possibleMoves();

			}
			
		else if ((isOccupied(move2)) && !(isOccupied(move2_2))) { 
			//jumpable(move2,move2_2);
			//turn=turn-1;
			//if (turn<1){turn=1};
			possibleMoves();
			};
	...
	function addScorePrompt () {
		console.log('*-------------------*');
		console.log('addScorePrompt');
		console.log('CALLED FROM: makeJump.')
		console.log('*-------------------*');
		console.log('');
		console.log('');		
		PUBNUB.publish({channel: "checkers", message: {type: 'score'}});

	}
	function addScore() {
		console.log('*-------------------*');
		console.log('addScore');
		console.log('CALLED FROM: pubNub.  CALLED FROM: addScorePrompt.')
		console.log('*-------------------*');
		console.log('');
		console.log('');		
		turn === 1 ? post=4 : post = turn-1;
		$('#score_'+post).text(player[post].score);
